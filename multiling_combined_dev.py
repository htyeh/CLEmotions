...
x_train = np.concatenate((en_train_data, es_train_data, hu_train_data, sk_train_data, sv_train_data, it_train_data, pt_train_data, de_train_data))
...
global_es_mic_tune = 0
global_es_mac_tune = 0

global_hu_mic_train = 0
global_hu_mac_train = 0
global_hu_mic_tune = 0
global_hu_mac_tune = 0

global_sk_mic_train = 0
global_sk_mac_train = 0
global_sk_mic_tune = 0
global_sk_mac_tune = 0

global_sv_mic_train = 0
global_sv_mac_train = 0
global_sv_mic_tune = 0
global_sv_mac_tune = 0

global_it_mic_train = 0
global_it_mac_train = 0
global_it_mic_tune = 0
global_it_mac_tune = 0

global_pt_mic_train = 0
global_pt_mac_train = 0
global_pt_mic_tune = 0
global_pt_mac_tune = 0
...
for i in range(num_iterations):
    ...
    gold_list.append(gold_en)
    pred_list.append(predicted_en)

    gold_de = y_test_de
    predicted_de = np.round(model.predict(x_test_de))
    gold_list.append(gold_de)
    pred_list.append(predicted_de)

    gold_es = y_test_es
    predicted_es = np.round(model.predict(x_test_es))
    gold_list.append(gold_es)
    pred_list.append(predicted_es)

    gold_hu = y_test_hu
    predicted_hu = np.round(model.predict(x_test_hu))
    gold_list.append(gold_hu)
    pred_list.append(predicted_hu)

    gold_sk = y_test_sk
    predicted_sk = np.round(model.predict(x_test_sk))
    gold_list.append(gold_sk)
    pred_list.append(predicted_sk)

    gold_sv = y_test_sv
    predicted_sv = np.round(model.predict(x_test_sv))
    gold_list.append(gold_sv)
    pred_list.append(predicted_sv)

    gold_it = y_test_it
    predicted_it = np.round(model.predict(x_test_it))
    gold_list.append(gold_it)
    pred_list.append(predicted_it)

    gold_pt = y_test_pt
    predicted_pt = np.round(model.predict(x_test_pt))
    gold_list.append(gold_pt)
    pred_list.append(predicted_pt)

    micro_list, macro_list = utils.multiling_evaluation(gold_list, pred_list)

    global_en_mic_train += micro_list[0]
    global_en_mac_train += macro_list[0]

    global_de_mic_train += micro_list[1]
    global_de_mac_train += macro_list[1]

    global_es_mic_train += micro_list[2]
    global_es_mac_train += macro_list[2]

    global_hu_mic_train += micro_list[3]
    global_hu_mac_train += macro_list[3]

    global_sk_mic_train += micro_list[4]
    global_sk_mac_train += macro_list[4]

    global_sv_mic_train += micro_list[5]
    global_sv_mac_train += macro_list[5]

    global_it_mic_train += micro_list[6]
    global_it_mac_train += macro_list[6]

    global_pt_mic_train += micro_list[7]
    global_pt_mac_train += macro_list[7]
    ...
    if FINETUNE:
        ...
        global_en_mic_tune += micro_list[0]
        global_en_mac_tune += macro_list[0]

        global_de_mic_tune += micro_list[1]
        global_de_mac_tune += macro_list[1]

        global_es_mic_tune += micro_list[2]
        global_es_mac_tune += macro_list[2]

        global_hu_mic_tune += micro_list[3]
        global_hu_mac_tune += macro_list[3]

        global_sk_mic_tune += micro_list[4]
        global_sk_mac_tune += macro_list[4]

        global_sv_mic_tune += micro_list[5]
        global_sv_mac_tune += macro_list[5]

        global_it_mic_tune += micro_list[6]
        global_it_mac_tune += macro_list[6]

        global_pt_mic_tune += micro_list[7]
        global_pt_mac_tune += macro_list[7]
...
es_micro_train = round( (global_es_mic_train/num_iterations), 4)
es_macro_train = round( (global_es_mac_train/num_iterations), 4)
hu_micro_train = round( (global_hu_mic_train/num_iterations), 4)
hu_macro_train = round( (global_hu_mac_train/num_iterations), 4)
sk_micro_train = round( (global_sk_mic_train/num_iterations), 4)
sk_macro_train = round( (global_sk_mac_train/num_iterations), 4)
sv_micro_train = round( (global_sv_mic_train/num_iterations), 4)
sv_macro_train = round( (global_sv_mac_train/num_iterations), 4)
it_micro_train = round( (global_it_mic_train/num_iterations), 4)
it_macro_train = round( (global_it_mac_train/num_iterations), 4)
pt_micro_train = round( (global_pt_mic_train/num_iterations), 4)
pt_macro_train = round( (global_pt_mac_train/num_iterations), 4)

print('{0: <20}'.format('En (micro/macro)') + '\t' + '{0: <10}'.format(en_micro_train) + '\t' + '{0: <10}'.format(en_macro_train))
print('{0: <20}'.format('De (micro/macro)') + '\t' + '{0: <10}'.format(de_micro_train) + '\t' + '{0: <10}'.format(de_macro_train))
print('{0: <20}'.format('Es (micro/macro)') + '\t' + '{0: <10}'.format(es_micro_train) + '\t' + '{0: <10}'.format(es_macro_train))
print('{0: <20}'.format('Hu (micro/macro)') + '\t' + '{0: <10}'.format(hu_micro_train) + '\t' + '{0: <10}'.format(hu_macro_train))
print('{0: <20}'.format('Sk (micro/macro)') + '\t' + '{0: <10}'.format(sk_micro_train) + '\t' + '{0: <10}'.format(sk_macro_train))
print('{0: <20}'.format('Sv (micro/macro)') + '\t' + '{0: <10}'.format(sv_micro_train) + '\t' + '{0: <10}'.format(sv_macro_train))
print('{0: <20}'.format('It (micro/macro)') + '\t' + '{0: <10}'.format(it_micro_train) + '\t' + '{0: <10}'.format(it_macro_train))
print('{0: <20}'.format('Pt (micro/macro)') + '\t' + '{0: <10}'.format(pt_micro_train) + '\t' + '{0: <10}'.format(pt_macro_train))
...
if FINETUNE:
    ...
    es_micro_tune = round( (global_es_mic_tune/num_iterations), 4)
    es_macro_tune = round( (global_es_mac_tune/num_iterations), 4)
    hu_micro_tune = round( (global_hu_mic_tune/num_iterations), 4)
    hu_macro_tune = round( (global_hu_mac_tune/num_iterations), 4)
    sk_micro_tune = round( (global_sk_mic_tune/num_iterations), 4)
    sk_macro_tune = round( (global_sk_mac_tune/num_iterations), 4)
    sv_micro_tune = round( (global_sv_mic_tune/num_iterations), 4)
    sv_macro_tune = round( (global_sv_mac_tune/num_iterations), 4)
    it_micro_tune = round( (global_it_mic_tune/num_iterations), 4)
    it_macro_tune = round( (global_it_mac_tune/num_iterations), 4)
    pt_micro_tune = round( (global_pt_mic_tune/num_iterations), 4)
    pt_macro_tune = round( (global_pt_mac_tune/num_iterations), 4)
    print('{0: <20}'.format('En (micro/macro)') + '\t' + '{0: <10}'.format(en_micro_tune) + '\t' + '{0: <10}'.format(en_macro_tune))
    print('{0: <20}'.format('De (micro/macro)') + '\t' + '{0: <10}'.format(de_micro_tune) + '\t' + '{0: <10}'.format(de_macro_tune))
    print('{0: <20}'.format('Es (micro/macro)') + '\t' + '{0: <10}'.format(es_micro_tune) + '\t' + '{0: <10}'.format(es_macro_tune))
    print('{0: <20}'.format('Hu (micro/macro)') + '\t' + '{0: <10}'.format(hu_micro_tune) + '\t' + '{0: <10}'.format(hu_macro_tune))
    print('{0: <20}'.format('Sk (micro/macro)') + '\t' + '{0: <10}'.format(sk_micro_tune) + '\t' + '{0: <10}'.format(sk_macro_tune))
    print('{0: <20}'.format('Sv (micro/macro)') + '\t' + '{0: <10}'.format(sv_micro_tune) + '\t' + '{0: <10}'.format(sv_macro_tune))
    print('{0: <20}'.format('It (micro/macro)') + '\t' + '{0: <10}'.format(it_micro_tune) + '\t' + '{0: <10}'.format(it_macro_tune))
    print('{0: <20}'.format('Pt (micro/macro)') + '\t' + '{0: <10}'.format(pt_micro_tune) + '\t' + '{0: <10}'.format(pt_macro_tune))
